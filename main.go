...
func gameStarter() {
	...
	entry := bytes.Index(resp, []byte("{"))
	idCh <- string(resp[7+entry : 39+entry])
	queCh <- []string{string(resp[53+entry : 78+entry]), string(resp[81+entry : 81+entry+bytes.Index(resp[81+entry:], []byte("\""))])}
